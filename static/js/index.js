...
function bakePie(mold, options) {
  ...
  function count() { //counting function. I can't tell if it's clever or totaly dumb, but it does the job.
    counter+=pieChunk;
    $(mold + " span").data("current", counter).text(Math.round(counter) / 10);  
      ...
      if (counter < almostDone & pieChunk > 0) { //stop iteratng 
        ...
        iterate = setTimeout(function(){
          $(mold + " span").data("current", options.percentage).text(options.percentage/10);
          clearTimeout(iterate);
        }, options.frame);
      }
  }
    if(options.percentage >= 0 && options.percentage <= 60){
        document.getElementById('npsLabel').style.color = '#CD0000';
        document.getElementById('npsLabel').innerHTML = 'Detractor';
        $(stuffing).css({
            stroke:'#CD0000'
        })
    }else if(options.percentage >= 70 && options.percentage <= 80){
        document.getElementById('npsLabel').style.color = '#FFB90F';
        document.getElementById('npsLabel').innerHTML = 'Neutral';
        $(stuffing).css({
            stroke:'#FFB90F'
        })
    }else if(options.percentage >= 90){
        document.getElementById('npsLabel').style.color = '#3fe687';
        document.getElementById('npsLabel').innerHTML = 'Promoter';
        $(stuffing).css({
            stroke:'#3fe687'
        })
    }
